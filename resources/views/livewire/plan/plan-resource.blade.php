<div>
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @forelse ($plans as $plan)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-6 text-left whitespace-nowrap">
                                        <div class="flex items-center">
                                            <span class="font-medium">{{ $plan->created_at->format('d/m/Y') }}</span>
                                        </div>
                                    </td>                                 
                                    <td class="py-3 px-6 text-left whitespace-nowrap flex justify-center">
                                        <div class="flex items-center">
                                            <x-jet-button wire:click="editPlan({{ $plan->id }})">
                                                {{ __('Edit') }}
                                            </x-jet-button>
                                            <x-jet-danger-button class="ml-2"
                                                wire:click="showDeleteModal({{ $plan->id }})">
                                                {{ __('Delete') }}
                                            </x-jet-danger-button>
                                        </div>
                                    </td>
    ...
    <x-jet-dialog-modal wire:model="showModal">
        ...
        <x-slot name="footer">
            ...
            @if ($planId)
                <x-jet-button class="ml-2" wire:click="updatePlan" wire:loading.attr="disabled">
                    {{ __('Update') }}
                </x-jet-button>
            @else
                <x-jet-button class="ml-2" wire:click="storePlan" wire:loading.attr="disabled">
                    {{ __('Create') }}
                </x-jet-button>
            @endif
        </x-slot>
